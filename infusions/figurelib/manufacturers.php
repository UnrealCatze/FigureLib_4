...
// Figure Cat Index
$figure_man_index = dbquery_tree(DB_FIGURE_MANUFACTURERS, 'figure_manufacturer_id', 'figure_manufacturer_parent');
...
if (
	...
	@list(
		$_GET['figure_manufacturer_id'],
		$figureTitle
	) = dbarraynum(dbquery("
		SELECT figure_manufacturer, figure_title FROM ".DB_FIGURE_ITEMS." WHERE figure_id='".intval($_GET['figure_id'])."' AND figure_freigabe='1' AND ".groupaccess("figure_visibility")."
	"));
...
// Display a Category
	...
	dbcount("(figure_manufacturer_id)", DB_FIGURE_MANUFACTURERS, "figure_manufacturer_id='".intval($_GET['figure_manufacturer_id'])."'")
) {
	...
	$cdata = dbarray(dbquery("
		...
		WHERE figure_manufacturer_id='".intval($_GET['figure_manufacturer_id'])."'
	"));
	...
	// Add Breadcrumbs
	figure_cat_breadcrumbs($figure_man_index);
